...
export default function ManageCoursePreviewPage(props) {
  ...
  const handleNextContent = () => {
    const currentIndex = course?.details.findIndex(
      (item) => item._id === activeContent._id
    );
    const nextIndex = currentIndex + 1;
    if (nextIndex < course?.details.length) {
      setActiveContent(course?.details[nextIndex]);
    }
  };
  return (
